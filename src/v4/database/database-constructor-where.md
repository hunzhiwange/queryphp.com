...
database::table ( 'test' )->where ( 'id', 2 )->where ( 'name', '>', '狗蛋' )->where ( 'value', 'like', '小鸭子' )->getAll ();
~~~

# 以数组直接传入
~~~
# SELECT `test`.* FROM `test` WHERE `test`.`name` LIKE '技术'
database::table ( 'test' )->where ( ['name','like', '技术'] )->getAll ();

# SELECT `test`.* FROM `test` WHERE `test`.`name` LIKE '技术' AND `test`.`value` <> '结局' 
database::table ( 'test' )->where ([['name','like', '技术'], ['value','<>', '结局'] ])->getAll ();
~~~

# OrWhere 语法
~~~
# SELECT `test`.* FROM `test` WHERE `test`.`name` LIKE '技术'
database::table('test')->where('name','like', '技术')->orWhere('value','<>', '结局')->getAll();
~~~
...
database::table('test')->whereLike('id','5')->getAll();

# SELECT `test`.* FROM `test` WHERE `test`.`id` LIKE '5'
database::table('test')->where('id','like', '5')->getAll();
~~~

# whereNotLike 语法
~~~
# SELECT `test`.* FROM `test` WHERE `test`.`id` NOT LIKE '5'
database::table('test')->whereNotLike('id','5')->getAll();

# SELECT `test`.* FROM `test` WHERE `test`.`id` NOT LIKE '5'
database::table('test')->where('id','not like', '5')->getAll();
~~~
...
database::table ( 'test')->whereNotExists ( function($objSelect){
          $objSelect->table ( 'subsql')->where('id',1);
      }
)
->getAll ();
